				...
				return new Promise((resolve, reject) => {
					...
					.then((stock) => {
						if (stock.type == '' || stock.type == null || stock.exchange == '' || stock.exchange == null || stock.sector == '' || stock.sector == null || stock.industry == '' || stock.industry == null) {
							...
							yahoo.quoteSummary(options.symbol, {modules:options.modules}).then((data) => {
								stock = {};
		...
		return new Promise(function(resolve, reject) {
			...
			symbols.forEach(function(symbol) {
				...
				.then(() => {
					if (time != undefined) {
						var now = new Date();
						console.log(sprintf('Symbol %s finished in %.1f seconds.', symbol, (now - time) / 1000, dateToString(from), dateToString(to)));	
					}
					return Promise.resolve();
